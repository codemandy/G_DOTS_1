...
def _nearest_neighbor_order(points: Sequence[Tuple[float, float]]) -> List[int]:
	if not points:
		return []
...
def write_svg_circles(
	*,
	layers: Iterable[Tuple[str, str, Sequence[Tuple[float, float]], float]],
	width_mm: float,
	height_mm: float,
	order: str,
	output_path: str,
	...
	for layer_name, color_hex, points, radius in layers:
		if not points:
			continue
		...
		for x, y in ordered:
			...
			parts.append(
				f'    <circle cx="{x:.4f}" cy="{y:.4f}" r="{radius:.4f}" fill="none" stroke="{stroke}" stroke-width="{radius/4:.4f}" />\n'
			)
		parts.append("  </g>\n")
